    ...
    <PageWrapper>
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
          ...
          {submissionsLoading || assessmentsLoading ? (
            <div className="p-6 text-sm text-slate-600">Loading...</div>
          ) : filteredSubmissions.length === 0 ? (
            <div className="p-6 text-sm text-slate-600">No submissions found.</div>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead className="bg-slate-50 border-b border-slate-200">
                  <tr>
                    ...
                    <th className="text-left px-6 py-3 font-semibold text-slate-700">Interviewee</th>
                ...
                <tbody>
                  {filteredSubmissions.map((s) => {
                    const assessment = assessments.find((a) => a.id === s.assessment_id);
                    const scorePct =
                      typeof s.score === "number" && typeof s.max_score === "number" && s.max_score > 0
                    ...
                    return (
                      <tr key={s.id} className="border-b border-slate-100 hover:bg-slate-50">
                        <td className="px-6 py-4">
                          <div>
                            <p className="font-medium text-slate-900">{assessment?.title || "-"}</p>
                            <p className="text-xs text-slate-500">ID: {s.assessment_id}</p>
                          </div>
                        </td>
                        <td className="px-6 py-4">
                          <div>
                        ...
                        <td className="px-6 py-4">
                          {scorePct !== null ? (
                            <div>
                              <p className="font-semibold text-slate-900">{scorePct}%</p>
                        ...
                        <td className="px-6 py-4">
                          <button
                            onClick={() => navigate(`/recruiter/submission/${s.id}`)}
